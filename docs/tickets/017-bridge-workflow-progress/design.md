...
| Component | Change |
|-----------|--------|
| `runtime/claude_adapter.py` | New `poll_workflow_progress() -> list[WorkflowProgress]` (parses `wf_<id>.json` + journal; fails soft) and `workflow_active() -> bool` (journal/state mtime liveness). The only code that knows the CC file layout. |
...
| `__main__.py` / `process/manager.py` | Construct + start the sweeper as a **tracked** task (ticket 008); expose `ProgressStore` to the web view model. |
...
| `runtime/transcript_reader.py` | **E2:** before raising `TimeoutError`, consult an injected `workflow_active` predicate; if a run is alive, reset the deadline. Replace the raw error string with a friendly message. Keep the existing sentinel/pending-tool ladder. |
| `runtime/pty_session.py` | Wire the adapter's `workflow_active` into the `TranscriptReader` it constructs. |
| `web/view_model.py` | Read `ProgressStore`; attach active runs per Lead; replace the always-zero `workers` count with an active-run indicator. |
...
```
 ...
 Fix: reader, at the deadline, asks adapter.workflow_active()? → True → reset.
      Key off where the work IS (the journal), not the proxy (the Lead transcript).
```

`workflow_active()` lives in the adapter (file-knowledge quarantine, fork B), so
the reader gains a predicate, not file-layout awareness.
