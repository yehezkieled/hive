...
- **2026-06-30** — Sprint 2026-Q2-S9: **completed Phase 4 (web dashboard → PWA).** Landed the async-ping layer: **Web Push** (041, #231, [ADR 0026](adr/0026-web-push-notification-channel.md)) — a 4th NotificationChannel filtering by kind, VAPID via env, a `HIVE_TELEGRAM_ALERTS` toggle to demote Telegram, service-worker `push`/`notificationclick` + client subscribe — **verified end-to-end on a real iPad** (installed PWA, backgrounded, native banners for "run finished" + "needs your decision" via Apple HTTP 201, tap → reply-ready deep-link, 048/#248). Plus 042 iPad polish + token→localStorage (#225), 043 standalone status-bar overlap (#221), 044 pattern-library awareness (#222), 045 CommandRouter registry + read-only Formatter split (#243), 046 scheduler-poke prose fix (#233). Carried → S10: 047 (pattern-skill invocation) + 049 (deep-link composer-clear polish). `ruff` + full `pytest -m "not integration"` green (1423 passed); deployed; Web Push live on the VPS. Closed early on goal-met (day 6 of 14). **Phase 4 done; S10 opens the finance-app dogfood + a Hive reliability/hardening track.**
- **2026-06-25** — Sprint 2026-Q2-S8: made the **web the primary control surface from an iPad** — shipped the full interaction layer and re-smoked it on an actual iPad. Responsive touch shell (037, #200, ADR 0022: drawer, 44px tap targets, safe-area insets, on-screen-keyboard handling, Enter-to-send hotfix); web decision-UI parity (038, #209, ADR 0024: `/api/decision` + durable entity-keyed store + SSE bubble + reply — answer a maestro 029 decision on-device); awaiting-you fleet view (039, #202: unread badge + waiting-on-me filter from `view_model`); PWA install (040, #205, ADR 0023: manifest, root-scoped service worker, apple meta + icons, HTTPS via `tailscale serve`, installed + standalone on iPad). 041 Web Push **spilled → S9** by design (large + gated by 040); 042/043 iPad-polish follow-ups opened for S9. `ruff` + full `pytest -m "not integration"` green; deployed. Closed early on goal-met (day 5 of 14). Phase 4 continues in S9 (Web Push + polish + the loop-engineering backend track opens).
- **2026-06-21** — Sprint 2026-Q2-S7: **completed Phase 3 (Workflow-native orchestration).** Shipped `debate` as the one Hive-native interaction-pattern recipe — full recipe in `role-lead.md`, asymmetric maestro menu, prompt-assembly tests, deployed + live-smoked (034, ADR 0020). Then **retired the named-library track**: `blackboard`/`tournament` (035/036) superseded — further coordination shapes ship as user-authored **global skills** (`~/.claude/skills`, inherited via 012/ADR 0008), zero engine work ([ADR 0021](adr/0021-further-patterns-as-global-skills.md)). Plus two independent loop-correctness gaps, both deployed + re-smoked: entity/team-name validation at all creation chokepoints (032, #186) and PA-Maestro self-identity in the prompt (033, #191). `ruff` + full `pytest -m "not integration"` green. Closed early on goal-met (window 06-16 → 06-30). Phase 4 (web/PWA "iPad daily driver") opens in S8.
